...
export default function SmokeyCursor({
  ...
  useEffect(() => {
    ...
    function applyInputs() {
      ...
      // 1. Process Mouse Interactions (runs every frame for zero-latency tracking)
      ...
      // 2. Process SpaceCanvas 3D Cube Interactions (Obstacles)
      // Throttled: we only need to update the obstacle forces every 3rd frame. 
      // Fluid physics interpolate this smoothly, but it saves ~240 GPU calls per second!
      if (frameCounter % 3 !== 0) return;

      const cubes = (window as any).__CUBES_2D_POSITIONS__;
      if (cubes && cubes.length) {
        cubes.forEach((cube: any) => {
          if (cube && cube.intensity > 0) {
            // We want the cubes to act as obstacles that the smoke flows around.
            // By injecting a subtle outward velocity from the cube's center (but NO dye/color),
            // when the user's mouse smoke hits the cube, the fluid will be forced to part and flow around it.
            
            // For a 3D feel, we inject velocity radially outward from the cube's center.
            // We'll do a small circular force to create a tiny eddy/vortex around the cube.
            // Increased multiplier slightly to compensate for running less often
            const forceMultiplier = 1.2;
            let vx = cube.vx * config.SPLAT_FORCE * forceMultiplier;
            let vy = cube.vy * config.SPLAT_FORCE * forceMultiplier;

            // Only apply the force if the cube is moving, or just a tiny bit constantly to keep the boundary active
            if (Math.abs(vx) > 1 || Math.abs(vy) > 1) {
               // Apply velocity ONLY, no color (black dye = {r:0,g:0,b:0})
               // This pushes the fluid around without emitting new smoke.
               splat(cube.x, cube.y, vx, vy, { r: 0, g: 0, b: 0 });
            }
          }
        });
      }
    ...
    function generateColor(): ColorRGB {
      // For Instagram colors, let's map a random value to the gradient directly!
      const t = Math.random();
      // Richer, deeper colors to avoid pale/light smoke
      const GRAD = [
        [255, 120, 0],   // vibrant deep orange
        [230, 20, 70],   // intense crimson
        [180, 10, 150],  // deep magenta
        [30, 40, 210],   // rich deep blue
      ];
      const scaled = t * (GRAD.length - 1);
      const i = Math.floor(scaled);
      const f = scaled - i;
      const a = GRAD[i], b = GRAD[i + 1] ?? GRAD[i];
      const r = a[0] + (b[0] - a[0]) * f;
      const g = a[1] + (b[1] - a[1]) * f;
      const b2 = a[2] + (b[2] - a[2]) * f;
